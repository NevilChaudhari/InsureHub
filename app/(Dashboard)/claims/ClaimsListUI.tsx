...
type Props = {
    claims: Claims[]
    totalClaims: number
    getClaims: (start: number, end: number) => void
    changeModetoDetails: (id: string) => void
}

export default function ClaimsListUI({ claims, totalClaims, getClaims, changeModetoDetails }: Props) {
    ...
    useEffect(() => {
        const calculatePages = () => {
            setPages(Math.ceil(totalClaims / 10))
        }
    ...
    useEffect(() => {
        setLoading(true)
        getClaims(activePage * 10 + 1, activePage * 10 + 10)
    }, [activePage])

    useEffect(() => {
        setLoading(false)
        console.log("agents updated in UI", claims);
    }, [claims]);
            ...
            <div className="flex gap-10">
                ...
                <div className="flex bg-white border border-[#E2E8F0] rounded-lg w-70 h-25 items-center p-3 place-content-between">
                    <div className="flex flex-col place-content-evenly h-full">
                        <label className="text-[#0F172A] font-semibold text-sm">Total Claims</label>
                        <label className="text-[#0F172A] font-bold text-2xl">{totalClaims}</label>
                    </div>
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="border-b border-gray-100 bg-gray-50/50">
                                ...
                                <th className="h-10 min-w-50 text-start px-5">Claim Amount</th>
                                <th className="h-10 min-w-50 text-start px-5">Status</th>
                                <th className="h-10 min-w-50 text-start px-5">Created At</th>
                            </tr>
                        </thead>
                        ...
                        {loading
                            ...
                            : (
                                <tbody>
                                    {claims.map((claim) => {
                                        return (
                                            <tr key={claim.id} className="border-b border-gray-50 font-medium text-[#0F172A] hover:bg-[#E5E7EB] cursor-pointer items-center">
                                                <td className="px-5 py-3">{claim.id}</td>
                                                <td className="px-5 py-3">{claim.contractId}</td>
                                                <td className="px-5 py-3 text-[#2563EB] font-semibold">{claim.claimAmount}</td>
                                                <td className="px-5 py-3">
                                                    <div className="flex w-20 h-7 items-center justify-center rounded-md bg-[#DCFCE7] text-[#16A34A]">{claim.status}</div>
                                                </td>
                                                <td className="px-5 py-3">{format(new Date(claim.created_at), "MMM d, yyyy")}</td>
                                            </tr>
                                        )
                                    })}
                                </tbody>
                ...
                <div className="flex place-content-between">
                    <div className="px-5 py-3 text-sm flex items-center min-w-100">
                        {totalClaims > 10 ? `Showing ${activePage * 10 + 1} to ${totalClaims < (activePage * 10 + 10) ? (totalClaims) : (activePage * 10 + 10)} out of ${totalClaims}` : `Showing ${totalClaims} out of ${totalClaims}`}
                    </div>
